...
## --------------------------------------------------------
		...
		#
	{
		...
		$this->fileExtension = $this->utility->fix_strtolower(strrchr($fileName, '.'));
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
		...
		#
	{
		...
		$cropPos = 'm';
		if (is_array($option) && $this->utility->fix_strtolower($option[0]) == 'crop')
		{
...
## --------------------------------------------------------
	...
	private function getCropPlacing($optimalWidth, $optimalHeight, $newWidth, $newHeight, $pos = 'm')
		...
		#
	{
		$pos = $this->utility->fix_strtolower($pos);
...
## --------------------------------------------------------
		...
		#
	{
		if (is_array($option))
		{
			if ($this->utility->fix_strtolower($option[0]) == 'crop' && count($option) == 2)
			{
				return 'crop';
			}
			else
		...
		if (is_string($option))
		{
			return $this->utility->fix_strtolower($option);
		}
		...
		#
	{
		if ($this->imageResized)
		{
			...
			if (is_string($value))
			{
				...
				$value = $this->utility->fix_strtolower($value);
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
		...
		#
	{
		...
		if ( ! is_array($bgColor))
		{
			if ($this->utility->fix_strtolower($bgColor) == 'transparent')
			{
				$isTransparent = true;
			}
		}
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function addCaptionBox($side = 'b', $thickness = 50, $padding = 0, $bgColor = '#000', $transaprencyAmount = 30)
		...
		#
	{
		$side = $this->utility->fix_strtolower($side);
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
		...
		#
	{
		...
		if ($this->utility->fix_strtolower(strrchr($watermarkImage, '.')) == '.png')
		{
...
## --------------------------------------------------------
		...
		#
	{
		...
		$extension = $this->utility->fix_strtolower($extension);
		switch ($extension)
		{
			case '.jpg':
			case '.jpeg':
...
## --------------------------------------------------------
		...
		#
	{
		...
		{
			if ($this->utility->fix_strtolower($value) == 'transparent')
			{
