...
use Rabies\FileManager\Utils\php_image_magician;

class Utility{

    public function someKarp(){
...
{
	$name = $this->fix_filename($name, $transliteration, false, '_', true);
	if (file_exists($old_path))
	{
 ...
 * @throws \Exception
 */
public function create_img($imgfile, $imgthumb, $newwidth, $newheight = null, $option = "crop")
{
 ...
 * @return  int
 */
public function filescount($path)
{
	$total_count = 0;
	$files = scandir($path);
	$cleanPath = rtrim($path, '/') . '/';
 ...
 */
public function create_folder($path = null, $path_thumbs = null)
{    
	$oldumask = umask(0);
 ...
 * @return  string
 */
public function fix_get_params($str)
{
 ...
 * @return string
 */
public function fix_filename($str, $transliteration, $convert_spaces = false, $replace_with = "_", $is_folder = false)
{
	if ($convert_spaces)
	{
...
public function fix_path($path, $transliteration, $convert_spaces = false, $replace_with = "_")
{
	$info = pathinfo($path);
 ...
 * @return  bool
 */
public function is_really_writable($dir)
{
	$dir = rtrim($dir, '/');
	// linux, safe off
 ...
 * @return  bool
 */
public function is_function_callable($name)
{
 ...
 * @param  bool    $is_rec
 */
public function rcopy($source, $destination, $is_rec = false)
{
	if (is_dir($source))
	{
		if ($is_rec === false)
 ...
 * @param  bool    $is_rec
 */
public function rrename($source, $destination, $is_rec = false)
{
	if (is_dir($source))
	{
		if ($is_rec === false)
...
public function rrename_after_cleaner($source)
{
	$files = scandir($source);
 ...
 * @param  bool    $is_rec
 */
public function rchmod($source, $mode, $rec_option = "none", $is_rec = false)
{
	if ($rec_option == "none")
	{
		chmod($source, $mode);
 ...
 * @return  bool
 */
public function chmod_logic_helper($perm, $val)
{
	$valid = array(
		1 => array( 1, 3, 5, 7 ),
		2 => array( 2, 3, 6, 7 ),
 ...
 * @param  bool    $halt
 */
public function debugger($input, $trace = false, $halt = false)
{
	ob_start();
